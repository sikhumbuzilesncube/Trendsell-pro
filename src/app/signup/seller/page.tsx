...
export default function SellerSignup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data: authData, error: authError } = await supabase.auth.signUp({
      ...
      options: { data: { phone: formData.phone, role: 'seller' } }
    })
    ...
    <main className="min-h-screen bg-stone-50 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h1 className="mt-6 text-3xl font-extrabold text-[#0B3D2E]">Join as a Seller</h1>
          <p className="mt-2 text-slate-700">Start selling without inventory. Set up your account in 2 minutes.</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div><label className="block text-sm font-semibold text-slate-900 mb-2">Store Name</label>
              <input type="text" name="storeName" required value={formData.storeName} onChange={handleChange}
                className="w-full px-4 py-3 border-2 border-stone-400 rounded-lg focus:ring-2 focus:ring-[#0B3D2E] focus:border-[#0B3D2E] outline-none transition text-slate-900 placeholder:text-slate-400"
                placeholder="e.g. John's Electronics" />
              <p className="mt-1 text-xs text-slate-600">This is what customers will see</p></div>

            <div className="border-t border-stone-200 pt-6">
              <label className="block text-sm font-semibold text-slate-900 mb-3">How should we pay you?</label>
              <div className="space-y-4">
                <div className="flex items-center">
                  <input type="radio" id="mobile" name="paymentMethod" value="mobile" checked={formData.paymentMethod === 'mobile'} onChange={handleChange} className="w-5 h-5 text-[#0B3D2E] focus:ring-[#0B3D2E]" />
                  <label htmlFor="mobile" className="ml-3 text-sm font-medium text-slate-900">Mobile Money</label>
                </div>
                ...
                <div className="flex items-center">
                  <input type="radio" id="bank" name="paymentMethod" value="bank" checked={formData.paymentMethod === 'bank'} onChange={handleChange} className="w-5 h-5 text-[#0B3D2E] focus:ring-[#0B3D2E]" />
                  <label htmlFor="bank" className="ml-3 text-sm font-medium text-slate-900">Bank Transfer</label>
                </div>
